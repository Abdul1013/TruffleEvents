...
> = {
  valid:     { bg: "bg-[#10B981]", icon: "✓", heading: "Valid Ticket" },
  duplicate: { bg: "bg-[#F59E0B]", icon: "⚠", heading: "Already Scanned" },
  invalid:   { bg: "bg-[#EF4444]", icon: "✕", heading: "Invalid Ticket" },
};
...
function ScannerPlaceholder() {
  return (
    <div className="flex h-72 w-full items-center justify-center rounded-xl bg-[#38240D]/10">
      <div className="h-8 w-8 animate-spin rounded-full border-4 border-[#C05800] border-t-transparent" />
    </div>
  );
}
...
export default function ScanPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Page header */}
      <div>
        <h2 className="text-3xl font-bold text-[#38240D]">QR Scanner</h2>
      ...
      {/* Scanner viewport */}
      {phase !== "idle" ? (
        <div className="relative overflow-hidden rounded-xl border-2 border-[#38240D] bg-black shadow-lg">
          <QrScanner
            onScan={handleScan}
          ...
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
            <div className="relative h-64 w-64">
              {[
                ...
                <div
                  key={i}
                  className={`absolute h-8 w-8 rounded-sm border-[#C05800] ${cls}`}
                />
              ))}
            </div>
          </div>
        ...
        <div className="rounded-xl border-2 border-[#C05800]/40 bg-white p-8 text-center shadow-sm">
          <div className="mb-6 text-6xl">📷</div>
          <h3 className="text-xl font-semibold text-[#38240D]">Ready to Scan</h3>
          <p className="mt-2 text-sm text-[#713600]">
          ...
          <button
            ...
            className="mt-6 rounded-lg bg-[#713600] px-8 py-3 font-semibold text-white hover:bg-[#C05800] transition-colors min-h-[44px]"
          >
            Start Scanner
          </button>
        </div>
      ...
      {phase !== "idle" && (
        <button
          ...
          className="w-full rounded-lg border-2 border-[#713600] py-3 font-semibold text-[#713600] hover:bg-[#FDFBD4] transition-colors min-h-[44px]"
        >
          Stop Scanner
        </button>
      )}
      ...
      <div className="grid grid-cols-3 gap-3 text-center text-sm">
        <div className="rounded-lg bg-[#10B981]/10 p-3 text-[#10B981] font-medium">
          <div className="text-xl mb-1">✓</div>
          Valid
        </div>
        <div className="rounded-lg bg-[#F59E0B]/10 p-3 text-[#F59E0B] font-medium">
          <div className="text-xl mb-1">⚠</div>
          Duplicate
        </div>
        <div className="rounded-lg bg-[#EF4444]/10 p-3 text-[#EF4444] font-medium">
          <div className="text-xl mb-1">✕</div>
          Invalid
        </div>
      </div>
