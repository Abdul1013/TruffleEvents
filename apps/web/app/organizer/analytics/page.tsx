...
function KpiCard({
  ...
  return (
    <div className="rounded-xl border-2 border-[#38240D] bg-white p-5">
      ...
      <p className="mt-2 text-3xl font-bold text-[#38240D]">{value}</p>
...
export default function OrganizerAnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold text-[#38240D]">Analytics</h2>
        <p className="mt-1 text-sm text-[#713600]">Sales and attendance for your events</p>
      </div>
        ...
        <Section title="Tickets Sold vs Remaining Capacity">
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={capacityData} margin={{ top: 0, right: 16, left: 0, bottom: 40 }}>
              ...
              <Bar dataKey="Remaining" stackId="a" fill="#38240D22" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </Section>
      ) : (
              ...
              <div className="space-y-2 text-sm">
                {statusBreakdown.map((s) => (
                  <div key={s.status} className="flex items-center gap-2">
                    ...
                    />
                    <span className="font-medium text-[#38240D]">{s.status}</span>
                    <span className="text-[#713600]/70">{s.count}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[#38240D]/10">
                {events.map((e) => (
                  <tr key={e.event_id} className="hover:bg-[#FDFBD4]/60">
                    <td className="py-3 pr-4 font-medium text-[#38240D]">
                      {e.title}
                      <span className="ml-2 text-xs text-[#713600]/60">
